...
export const PromptBuilder: React.FC = () => {
  ...
  return ( <
    div className = "flex flex-col h-full" > { /* Header row for add-block buttons */ } <
    ...
    <
    button onClick = { handleAddTextBlock } className = "px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" >
    Add Text Block <
    /button> <
    button onClick = { handleAddTemplateBlock } className = "px-4 py-2 bg-purple-500 text-white rounded hover:bg-purple-600" >
    Add Template Block <
    /button> <
    button onClick = { handleAddFileBlock } className = "px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600" >
    Add File Block <
    /button> <
    button onClick = { togglePreview } className = "px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600" >
    ...
    { /* If the preview is shown, we have a drag handle and a preview area. */ } {
      showPreview && ( <
        ...
        div className = "preview-drag-handle h-2"
        onMouseDown = { onMouseDownPreviewHandle }
        />

        { /* Preview container with scroll */ } <
        div className = "bg-white dark:bg-gray-700 border-t border-gray-300 dark:border-gray-600 overflow-auto"
    ...
    TemplateSelectorModal isOpen = { showTemplateModal } onClose = {
      () => setShowTemplateModal(false) } onInsertBlocks = { handleInsertTemplateBlocks }
    /> <
    /div>
  );
};
